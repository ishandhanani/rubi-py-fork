...
class AidProcessing:
    ...
    def recreate_aid_history(self, aid, bin_size=60, raw=False):
        ...
        for asset, group in aid_history_grouped:
            ...
            group = group.sort_values(
                by=["block", "block_index", "timestamp"]
            ...
            credits = (
                ...
                .agg({"credits_debits": "sum"})
                .reset_index()
            )
            credits_raw = (
                group.groupby(["timestamp"])
                .agg({"credits_debits_raw": "sum"})
                .reset_index()
            )

            # fill in the credits and debits for each timestamp
            history[f"{asset}_credits"] = (
                history["timestamp"]
                .map(credits.set_index("timestamp")["credits_debits"].to_dict())
                .fillna(0)
            )
            history[f"{asset}_credits_raw"] = (
                history["timestamp"]
                .map(credits_raw.set_index("timestamp")["credits_debits_raw"].to_dict())
                .fillna(0)
            )
            ...
            history[f"{asset}_balance"] = (
                history["timestamp"]
                .map(filtered_group.set_index("timestamp")["balance"].to_dict())
                .fillna(method="ffill")
                .fillna(0)
            )
            history[f"{asset}_balance_raw"] = (
                history["timestamp"]
                .map(filtered_group.set_index("timestamp")["balance_raw"].to_dict())
                .fillna(method="ffill")
                .fillna(0)
            )
            ...
            prices = self.price.get_price_in_range(
                start=min_timestamp, end=max_timestamp, pair=ticker
            )

            # fill in the price for each asset at the beginning of each minute period
            history[f"{asset}_price"] = (
                history["timestamp"]
                .map(prices)
                .fillna(method="ffill")
                .fillna(method="bfill")
    ...
    def evaluate_aid_performance(self, data, assets, start_time=None, end_time=None):
        ...
        for asset in assets:
            ...
            data[f"{asset}_balance_delta"] = (
                data[f"{asset}_balance"] - data[f"{asset}_credits_total"]
            )
            data[f"{asset}_balance_delta_raw"] = (
                data[f"{asset}_balance_raw"] - data[f"{asset}_credits_total_raw"]
            )

            # calculate the usd value of the balance, credits, and delta
            data[f"{asset}_balance_usd"] = (
                data[f"{asset}_balance"] * data[f"{asset}_price"]
            )
            data[f"{asset}_credits_total_usd"] = (
                data[f"{asset}_credits_total"] * data[f"{asset}_price"]
            )
            data[f"{asset}_balance_delta_usd"] = (
                data[f"{asset}_balance_delta"] * data[f"{asset}_price"]
            )

        data["total_balance_usd"] = data[
            [f"{asset}_balance_usd" for asset in assets]
        ].sum(axis=1)
        data["total_credits_usd"] = data[
            [f"{asset}_credits_total_usd" for asset in assets]
        ].sum(axis=1)
        data["total_balance_delta_usd"] = data[
            [f"{asset}_balance_delta_usd" for asset in assets]
        ].sum(axis=1)

        return {"data": data, "assets": assets}

    def analyze_aid_history(
        self, aid, start_time=None, end_time=None, bin_size=60, raw=False
    ):
        ...
        # get the aid history
        df = self.market_aid.get_aid_history(
            aid=aid, start_time=start_time, end_time=end_time, bin_size=bin_size
        )
        ...
        for asset, ticker in zip(assets, tickers):
            price_data[asset] = self.price.get_price_in_range(
                start=min_timestamp, end=max_timestamp, pair=ticker
            )
        ...
        for asset, group in df_grouped:
            ...
            if raw:
                asset_deltas[asset] = group.set_index("timestamp").to_dict()[
                    "delta_raw"
                ]
                asset_balances[asset] = group.set_index("timestamp").to_dict()[
                    "balance_raw"
                ]
                credits_debits[asset] = group.set_index("timestamp").to_dict()[
                    "credits_debits_raw"
                ]
            else:
        ...
        for asset in assets:
            history[f"{asset}_balance"] = (
                history["timestamp"]
                .map(asset_balances[asset])
                .fillna(method="ffill")
                .fillna(0)
            )

        for asset in assets:
            history[f"{asset}_hodl_balance"] = (
                history["timestamp"]
                .map(credits_debits[asset])
                .fillna(method="ffill")
                .fillna(0)
            )

        for asset in assets:
            history[f"{asset}_delta"] = (
                history["timestamp"]
                .map(asset_deltas[asset])
                .fillna(method="ffill")
                .fillna(0)
            )
        ...
        for asset in assets:
            history[f"{asset}_balance_usd"] = (
                history[f"{asset}_balance"] * history[f"{asset}_price"]
            )

        for asset in assets:
            history[f"{asset}_hodl_balance_usd"] = (
                history[f"{asset}_hodl_balance"] * history[f"{asset}_price"]
            )

        for asset in assets:
            history[f"{asset}_delta_usd"] = (
                history[f"{asset}_delta"] * history[f"{asset}_price"]
            )

        # compute the total balance and delta
        history["total_balance_usd"] = history[
            [f"{asset}_balance_usd" for asset in assets]
        ].sum(axis=1)
        history["total_hodl_balance_usd"] = history[
            [f"{asset}_hodl_balance_usd" for asset in assets]
        ].sum(axis=1)
        history["total_delta_usd"] = history[
            [f"{asset}_delta_usd" for asset in assets]
        ].sum(axis=1)
    ...
    def aid_performance_evaluation(self, data, tokens, asset_mix=None):
        ...
        for token in tokens:
            ...
            data[f"{token}_balance_initial_usd"] = (
                data[f"{token}_balance_initial"] * data[f"{token}_price"]
            )
            data["total_balance_initial_usd"] += data[f"{token}_balance_initial_usd"]

        # compute the hodl strat performance delta
        data["hodl_strat_performance_delta"] = (
            data["total_balance_usd"] - data["total_balance_initial_usd"]
        )
        ...
        if asset_mix:
            ...
            for token in tokens:
                ...
                ideal_asset_balances[token] = (
                    (initial_usd_balance)
                    * asset_mix[token]
                    / initial_asset_prices[token]
                )
                data[f"{token}_balance_ideal"] = ideal_asset_balances[token]
                data[f"{token}_balance_ideal_usd"] = (
                    data[f"{token}_balance_ideal"] * data[f"{token}_price"]
                )
                data["total_balance_ideal_usd"] += data[f"{token}_balance_ideal_usd"]

            # compute the asset mix strat performance delta
            data["asset_mix_strat_performance_delta"] = (
                data["total_balance_usd"] - data["total_balance_ideal_usd"]
            )

        return data

    def aid_fill_tracking(
        self,
        aid,
        asset,
        quote,
        ...
        if len(offers) == 0:
            return pd.DataFrame(
                columns=[
                    "time_bin",
                    "direction",
                    "offers_pay_amt_formatted",
                    "offers_paid_amt_formatted",
                    "offers_buy_amt_formatted",
                    "offers_bought_amt_formatted",
                    "price",
                    "offers_live",
                ]
            )
        ...
        if len(quote) < 6:
            asks = offers[
                (offers["offers_pay_gem_symbol"] == asset.upper())
                & (offers["offers_buy_gem_symbol"] == quote.upper())
            ]
            bids = offers[
                (offers["offers_pay_gem_symbol"] == quote.upper())
                & (offers["offers_buy_gem_symbol"] == asset.upper())
            ]
        else:
            asks = offers[
                (offers["offers_pay_gem_id"] == asset.lower())
                & (offers["offers_buy_gem_id"] == quote.lower())
            ]
            bids = offers[
                (offers["offers_pay_gem_id"] == quote.lower())
                & (offers["offers_buy_gem_id"] == asset.lower())
            ]

        # group the offers by the time bin they occurred in
        asks["time_bin"] = asks["offers_transaction_timestamp"].apply(
            lambda x: int((x // bin_size) * bin_size)
        )
        bids["time_bin"] = bids["offers_transaction_timestamp"].apply(
            lambda x: int((x // bin_size) * bin_size)
        )
        ...
        asks["price"] = (
            asks["offers_buy_amt_formatted"] / asks["offers_pay_amt_formatted"]
        )  # price to buy the asset from the seller
        bids["price"] = (
            bids["offers_pay_amt_formatted"] / bids["offers_buy_amt_formatted"]
        )  # price to sell the asset to the buyer
