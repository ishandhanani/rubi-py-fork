...
class Gas:
    ...
    def get_optimism_txn_gas_data(self, txn, priced=True):
        ...
        txn_data["l1_fee"] = int(
            txn_data["l1_gas_used"]
            * txn_data["l1_gas_price"]
            * txn_data["l1_fee_scalar"]
        )
        ...
        if priced:
            ...
            txn_data["total_fee_usd"] = (
                txn_data["total_fee_eth"] * txn_data["eth_price"]
            )
    ...
    async def get_optimism_txns_gas_data(self, txns):
        ...
        tasks = [
            loop.run_in_executor(None, self.get_optimism_txn_gas_data, txn, False)
            for txn in txns
        ]
    ...
    def txn_dataframe_update(
        ...
        if price_query == "range":
            ...
            for timestamp in timestamps:
                eth_price_data[timestamp] = self.get_closest_timestamp_value(
                    eth_prices, timestamp
                )
        ...
        if total_fee_eth:
            txn_dataframe["total_fee_eth"] = txn_dataframe[txn_column].map(
                lambda x: txn_gas_data[x]["total_fee_eth"]
            )
        if total_fee_usd:
            txn_dataframe["total_fee_usd"] = txn_dataframe.apply(
                lambda x: txn_gas_data[x[txn_column]]["total_fee_eth"]
                * eth_price_data[x[timestamp_column]],
                axis=1,
            )
        if l2_gas_price:
            txn_dataframe["l2_gas_price"] = txn_dataframe[txn_column].map(
                lambda x: txn_gas_data[x]["l2_gas_price"]
            )
        if l2_gas_used:
            txn_dataframe["l2_gas_used"] = txn_dataframe[txn_column].map(
                lambda x: txn_gas_data[x]["l2_gas_used"]
            )
        if l1_gas_used:
            txn_dataframe["l1_gas_used"] = txn_dataframe[txn_column].map(
                lambda x: txn_gas_data[x]["l1_gas_used"]
            )
        if l1_gas_price:
            txn_dataframe["l1_gas_price"] = txn_dataframe[txn_column].map(
                lambda x: txn_gas_data[x]["l1_gas_price"]
            )
        if l1_fee_scalar:
            txn_dataframe["l1_fee_scalar"] = txn_dataframe[txn_column].map(
                lambda x: txn_gas_data[x]["l1_fee_scalar"]
            )
        if l1_fee:
            txn_dataframe["l1_fee"] = txn_dataframe[txn_column].map(
                lambda x: txn_gas_data[x]["l1_fee"]
            )
        if l2_fee:
            txn_dataframe["l2_fee"] = txn_dataframe[txn_column].map(
                lambda x: txn_gas_data[x]["l2_fee"]
            )
        if total_fee:
            txn_dataframe["total_fee"] = txn_dataframe[txn_column].map(
                lambda x: txn_gas_data[x]["total_fee"]
            )
        if l1_fee_eth:
            txn_dataframe["l1_fee_eth"] = txn_dataframe[txn_column].map(
                lambda x: txn_gas_data[x]["l1_fee_eth"]
            )
        if l2_fee_eth:
            txn_dataframe["l2_fee_eth"] = txn_dataframe[txn_column].map(
                lambda x: txn_gas_data[x]["l2_fee_eth"]
            )
        if eth_price:
            txn_dataframe["eth_price"] = txn_dataframe.map(
                lambda x: eth_price_data[x[timestamp_column]]
            )
        if l1_fee_usd:
            txn_dataframe["l1_fee_usd"] = txn_dataframe.map(
                lambda x: txn_gas_data[x[txn_column]]["l1_fee_eth"]
                * eth_price_data[x[timestamp_column]]
            )
        if l2_fee_usd:
            txn_dataframe["l2_fee_usd"] = txn_dataframe.map(
                lambda x: txn_gas_data[x[txn_column]]["l2_fee_eth"]
                * eth_price_data[x[timestamp_column]]
            )
