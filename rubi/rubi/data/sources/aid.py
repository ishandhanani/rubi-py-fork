...
class AidData:
    ...
    def get_aid_history(
        ...
        AidTokenHistory.balance_formatted = (
            AidTokenHistory.balance / 10**AidTokenHistory.aid_token.token.decimals
        )
        AidTokenHistory.balance_change_formatted = (
            AidTokenHistory.balance_change
            / 10**AidTokenHistory.aid_token.token.decimals
        )
        ...
        if where:
            histories = self.market_aid.Query.aidTokenHistories(
        ...
        if df.empty:
            ...
            df = df.sort_values(["block", "block_index", "index"]).reset_index(
                drop=True
            )
    ...
    def get_aid_txns(self, aid=None, start_time=None, end_time=None, first=1000000000):
        ...
        field_paths = [txns.id, txns.timestamp, txns.aid.id]

        df = self.subgrounds.query_df(field_paths, pagination_strategy=ShallowStrategy)

        return df

    def get_book_updates(
        self, aid=None, asset=None, start_time=None, end_time=None, first=1000000000
    ):
        ...
        BookUpdate.amount_formatted = (
            BookUpdate.amount / 10**BookUpdate.aid_token.token.decimals
        )
...
class SuperAidData(AidData):
    ...
    def get_aid_txns_gas_data(
        ...
        df = self.gas.txn_dataframe_update(
            txns,
            "transactions_id",
            ...
            l2_fee_eth=l2_fee_eth,
            eth_price=eth_price,
            l1_fee_usd=l1_fee_usd,
            l2_fee_usd=l2_fee_usd,
        )

        return df

    def get_aid_gas_spend_binned(
        self,
        aid=None,
        start_time=None,
        end_time=None,
        ...
        gas_spend = self.get_aid_txns_gas_data(
            aid,
            start_time,
            end_time,
            total_fee_eth,
        ...
        # bin the data
        gas_spend["time_bin"] = gas_spend["transactions_timestamp"].apply(
            lambda x: (x // granularity) * granularity
        )
